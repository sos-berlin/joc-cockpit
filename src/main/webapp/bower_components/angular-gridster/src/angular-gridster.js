...
(function(root, factory) {
	...
	.constant('gridsterConfig', {
		...
		margins: [22, 22], // margins in between grid items
		outerMargin: true,
		sparse: false, // "true" can increase performance of dragging and resizing for big grid (e.g. 20x50)
		isMobile: false, // toggle mobile view
		mobileBreakPoint: 786, // width threshold to toggle mobile mode
		...
		draggable: { // options to pass to draggable handler
			enabled: true,
			scrollSensitivity: 30, // Distance in pixels from the edge of the viewport after which the viewport should scroll, relative to pointer
			scrollSpeed: 15 // Speed at which the window should scroll once the mouse pointer gets within scrollSensitivity distance
		}
	})

	.controller('GridsterCtrl', ['gridsterConfig','$scope', '$timeout',
		function(gridsterConfig,$scope, $timeout) {
            ...
            this.removeItem = function (item) {
                ...
                if (arr.length > 0) {
                    for (var i = 0; i < arr.length; i++) {
                        if (arr[i].name == item.$element.attr('id')) {
                            arr.splice(i, 1);
                            break;
                        }
                    }
		...
		return {
			...
			link: function(scope, $el, attrs, gridster) {
				...
				scope.previewStyle = function() {
					...
					return {
						display: 'block',
						height: gridster.movingItem.$element.css('height'),
						width: (gridster.movingItem.sizeX * gridster.curColWidth - gridster.margins[1]) + 'px',
						top: gridster.movingItem.$element.css('top'),
						left: gridster.movingItem.$element.css('left')
					};
				};
			}
		};
				...
				compile: function($tplElem) {
					...
					return function(scope, $elem, attrs, gridster) {
						...
						function updateHeight() {
							...
							$elem.css('height', (gridster.gridHeight * gridster.curRowHeight) + 'px');
							$elem.css('height', sum + 'px');
						}
	...
	.controller('GridsterItemCtrl', function() {
		...
		this.setElementPosition = function() {
			if (this.gridster.isMobile) {
				...
				var x = {
					row: this.row,
					height: this.$element.attr('id') == 'agentClusterRunningTasks' ? 228 : this.$element.innerHeight(),
