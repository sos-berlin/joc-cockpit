...
angular.module('ngSanitize',[]).provider('$sanitize',$SanitizeProvider);angular.module('ngSanitize').filter('linky',['$sanitize',function($sanitize){var LINKY_URL_REGEXP=/((ftp|https?):\/\/|(www\.)|(mailto:)?[A-Za-z0-9._%+-]+@)\S*[^\s.;,(){}<>"\u201d\u2019]/i,MAILTO_REGEXP=/^mailto:/i;var linkyMinErr=angular.$$minErr('linky');var isDefined=angular.isDefined;var isFunction=angular.isFunction;var isObject=angular.isObject;var isString=angular.isString;return function(text,target,attributes){if(text==null||text==='')return text;if(!isString(text))throw linkyMinErr('notstring','Expected string but received: {0}',text);var pattern=/<a\s.*?<\/a>/g;var index=text.search(pattern);var x=[];if(index!=-1){text.match(pattern).map(function(val,index){x.push(val);text=text.replace(val,'$SOS$'+index)})}
...
addText(raw);if(index!=-1){var temp=html.join('');for(var j=0;j<x.length;j++){temp=temp.replace('$SOS$'+j,x[j]);temp=temp.replace(/<a/g,'<a class="text-u-l text-hover-primary" target="_blank" ')}
return $sanitize(temp)}
return $sanitize(html.join(''));function addText(text){if(!text){return}
html.push(sanitizeText(text))}
function addLink(url,text){var key,linkAttributes=attributesFn(url);html.push('<a class="text-u-l text-hover-primary"');for(key in linkAttributes){html.push(key+'="'+linkAttributes[key]+'" ')}
html.push('target="','_blank','" ');html.push('href="',url.replace(/"/g,'&quot;'),'">');addText(text);html.push('</a>')}}}])})(window,window.angular)
