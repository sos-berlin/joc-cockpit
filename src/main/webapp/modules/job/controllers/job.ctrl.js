    ...
    function JobWorkflowCtrl($scope, $rootScope, $uibModal, CoreService, ConditionService, AuditLogService, gettextCatalog, $timeout, toasty, orderBy, FileSaver, $filter, DailyPlanService, JobService) {
        ...
        function recursivelyConnectJobs(reload, checkScroll, cb) {
            ...
            angular.forEach(vm.allJobs, function (job) {
                if(!job.path1) {
                    job.path1 = job.path.substring(0, job.path.lastIndexOf('/')) || '/';
                }
            ...
            if (jobPaths.length > 0) {
                ...
                ConditionService.inCondition({
                    ...
                    if(result1) {
                        mergeConditions(res, result1, reload, checkScroll, tempJobs, cb);
                    }else{
                        res1 = res;
                    }
                }, function () {
                ...
                ConditionService.outCondition({
                    ...
                    if(res1) {
                        mergeConditions(res1, result, reload, checkScroll, tempJobs, cb);
                    }else{
                        result1 = result;
                    }
                }, function () {
        ...
        function mergeConditions(res, result, reload, checkScroll, tempJobs, cb){
            vm.workflows = [];
        ...
        vm.setRuntimeJobstream = function (jobStream, starter) {
            ...
            if (jobStream) {
                ...
                let data = JSON.parse(jobStream.cell.getAttribute('starter'));
                vm.order = {runTime: data.runTime, path: jobStream.cell.getAttribute('label'), isJobStream: true};
            } else {
        ...
        vm.$on('Close-Jobstream-Model', function (evt, arg) {
            if (arg === 'ok') {
                if (vm._jobstream) {
                    ...
                    let data = JSON.parse(vm._jobstream.cell.getAttribute('starter'));
        ...
        function addJobStream(jobStream, isRename) {
            let path = vm.allJobs[0].path1;
            if (path.substring(0, 1) !== '/') {
                path = '/' + path;
            }
        ...
        vm.updateWorkflow = function (jobStream) {
            ...
            modalInstance.result.then(function () {
                ...
                let isRename;
                if(jobStream.jobStream !== vm._jobStream.jobStream){
                    isRename = jobStream.jobStream;
                }
        ...
        $scope.$on('importJobStream', function () {
            vm.getJobStreams(function () {
                setTimeout(function () {
                    recursivelyConnectJobs(vm.jobStreamList.length > 0, vm.jobStreamList.length > 0);
                }, 10);
            });
        });
