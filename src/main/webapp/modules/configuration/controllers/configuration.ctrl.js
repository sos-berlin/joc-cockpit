...
(function () {
    ...
    JOEEditorCtrl.$inject = ['$scope', 'SOSAuth', 'CoreService', 'EditorService', 'orderByFilter', '$uibModal', 'clipboard', 'gettextCatalog'];

    function JOEEditorCtrl($scope, SOSAuth, CoreService, EditorService, orderBy, $uibModal, clipboard, gettextCatalog) {
        const vm = $scope;
        vm.tree = [];
        vm.filter_tree = {};
        vm.filterTree1 = [];
        ...
        function recursiveTreeUpdate(scrTree, destTree) {
            ...
            if (scrTree && destTree) {
                for (let i = 0; i < destTree.length; i++) {
                    for (let j = 0; j < scrTree.length; j++) {
                        if (destTree[i].path === scrTree[j].path) {
                            destTree[i].deleted = scrTree[j].deleted;
                            destTree[i].lockedBy = scrTree[j].lockedBy;
                            if (destTree[i].deleted) {
                                destTree[i].expanded = false;
                            }
            ...
            if (scrTree && scrTree.length > 0 && angular.isArray(destTree)) {
                angular.forEach(scrTree, function (data) {
                    destTree.push(data);
                });
            }
            return destTree;
                        ...
                        for (let j = 0; j < tempArray.length; j++) {
                            if (tempArray[j].path && tempArray[j].path.match(regex)) {
                                if (array[i].path && tempArray[j].path.split('/').length == (array[i].path.split('/').length + 1)) {
                                    if (array[i].folders) {
                                        if (array[i].folders.filter(function (e) {
                                            return e.name === tempArray[j].name
                                        }).length == 0) {
                                            ...
                                            let x = array[i].folders.filter(function (e) {
                                                return e.name === tempArray[j].name
                                            });
        ...
        function checkFolder(tempArray) {
            if (tempArray.path != undefined) {
                let len = tempArray.path.split('/').length - 1;
                let path = tempArray.path.split('/');
                if (path[len - (len - 1)]) {
                    let a = [];
                    if (vm.deployables[0].folders.length > 0) {
                        a = vm.deployables[0].folders.filter(function (e) {
                            return e.name === path[len - (len - 1)];
                        });
                    }
                    if (a == 0) {
                        len--;
                        ...
                        if (length > 0) {
                            createFolder(len, vm.deployables[0].folders[vm.deployables[0].folders.length - 1], tempArray, path);
                        } else {
                            vm.deployables[0].folders[vm.deployables[0].folders.length - 1].push(tempArray);
                        }
                    } else {
                        haveFolder(a[0], tempArray, path, path.length - 2);
                    }
                }
            }
        }

        function haveFolder(array, tObj, pathArray, len) {
            let a = [];
            if (array.folders.length > 0) {
                a = array.folders.filter(function (e) {
                    return e.name === pathArray[len];
                });
            }
            if (a.length > 0) {
                len++;
                haveFolder(a[0], tObj, pathArray, len);
            } else {
                len++;
                if (len == pathArray.length) {
                    tObj = Object.assign(tObj, {folders: []});
                    array.folders.push(tObj);
                } else if (len < pathArray.length) {
                    let a = [];
                    if (array.folders.length > 0) {
                        len--;
                        a = array.folders.filter(function (e) {
                            return e.name === pathArray[len - 1];
                        });
                    }
                    if (a.length > 0) {
                        haveFolder(a[0], tObj, pathArray, len);
        ...
        function createFolder(length, array, tObj, pathArray) {
            ...
            if (array.folders.length > 0) {
                a = array.folders.filter(function (e) {
                    return e.name === pathArray[length - 1];
                });
            }
            if (a.length === 0) {
                array.folders.push({
                    name: pathArray[length - 1],
                    path: array.path + '/' + pathArray[length - 1],
                    folders: []
                });
                length++;
                if (length < pathArray.length) {
                    createFolder(length, array.folders[array.folders.length - 1], tObj, pathArray);
                } else {
                    let x = array.folders.filter(function (e) {
                        return e.name === pathArray[length - 2];
                    });
                    x[0].folders.push(tObj);
                }
            }
        }
                ...
                for (let i = 0; i < vm.deployableObjects[type].length; i++) {
                    ...
                    if (flag) {
                        objArr.push({
                            ...
                            path: vm.deployableObjects[type][i].path,
                        ...
                        if (tempArray[j].path.split('/').length == (array[i].path.split('/').length + 1)) {
                            if (array[i].folders) {
                                if (array[i].folders.filter(function (e) {
                                    ...
                                    var x = array[i].folders.filter(function (e) {
                                        return e.name === tempArray[j].name
                                    });
                                    x[0] = Object.assign(x[0], tempArray[j]);
                                    tempArray.splice(j, 1);
        ...
        function checkObjectRecursively(data) {
            ...
            let allParentFolders = [], _splitArr = [];
            if (mainFolder && mainFolder.path)
                _splitArr = mainFolder.path.split('/');
            if (_splitArr.length > 1) {
                _splitArr.splice(_splitArr.length - 1, 1);
                let str = '';
            ...
            if (isExit) {
                if (!checkMissingObject(mainFolder, data.type)) {
                    ...
                    for (let i = 0; i < allParentFolders.length; i++) {
                        for (let j = 0; j < vm.deployableObjects.folders.length; j++) {
                            if (vm.deployableObjects.folders[j] === allParentFolders[i]) {
                                vm.deployableObjects.folders.splice(j, 1);
                                break;
                            }
                        }
                    }
                ...
                if (checkMissingObject(mainFolder, data.type)) {
                    ...
                    if (isAdd) {
                        ...
                        for (let i = 0; i < allParentFolders.length; i++) {
                            if (vm.deployableObjects.folders.indexOf(allParentFolders[i]) === -1 && allParentFolders[i] !== '/') {
                                vm.deployableObjects.folders.push(allParentFolders[i]);
                            }
                        }
                    }
                }
        ...
        function mergeFolderData(sour, dest, type) {
            ...
            for (let i = dest.children.length - 1; i >= 0; i--) {
                if (dest.children[i].match) {
                    ...
                    console.log('Else...', dest.children[i].name, dest.children[i].type);
        ...
        function updateFolders(path, recursive) {
            if (vm.tree.length > 0) {
        ...
        let lastClickedItem = null,
            defaultObjects = ['name', 'deleted', 'deployed', 'selected1', 'children', 'type', '$$hashKey', 'expanded', 'orderId', 'jobChain'];
        ...
        vm.treeHandler = function (data, evt) {
            ...
            if (data.type) {
                ...
                if (_path && evt.$parentNodeScope.$modelValue && evt.$parentNodeScope.$modelValue.type) {
                    lastClickedItem = evt.$parentNodeScope.$modelValue;
                    vm.getFileObject(lastClickedItem, _path, function () {
                        broadcastData(data, evt);
                    })
                } else {
                    setTimeout(function () {
        ...
        vm.createNewMonitor = function (object, parent, evt) {
            ...
            obj.parent = parent;
            vm.storeObject(obj, {ordering: obj.ordering}, evt, function (result) {
                if (!result)
                    object.push(obj);
            });
        };
        ...
        vm.removeObject = function (object, evt) {
            ...
            vm.deleteObject(object.type || 'FOLDER', name, path);
        };

        vm.deleteObject = function (objectType, name, path) {
            if (name) {
                if (!path) {
                    path = vm.path
                }
        ...
        function deployWithComment(obj) {
            ...
            if (vm.events && vm.events[0] && vm.events[0].eventSnapshots) {
                for (let i = 0; i < vm.events[0].eventSnapshots.length; i++) {
                    if (vm.events[0].eventSnapshots[i].eventType.match(/FileBase/) && !vm.events[0].eventSnapshots[i].eventId && vm.isloaded) {
                        init(vm.events[0].eventSnapshots[i].path.substring(0, vm.events[0].eventSnapshots[i].path.lastIndexOf('/') + 1));
                        break
                        ...
                        if (vm.events[0].eventSnapshots[i].objectType === 'FOLDER') {
                            let _parent = vm.events[0].eventSnapshots[i].path.substring(0, vm.events[0].eventSnapshots[i].path.lastIndexOf('/')) || '/';
                            init(_parent);
                        } else {
    ...
    function JobEditorCtrl($scope, $uibModal, EditorService, $interval) {
        ...
        vm.removeJob = function (job) {
            for (let i = 0; i < vm.jobs.length; i++) {
                if (vm.jobs[i].name === job.name) {
                    vm.deleteObject('JOB', job.name, job.path);
                    break;
                }
            }
        };
        ...
        vm.openSidePanel = function (title) {
            ...
            if (title === 'parameter') {
                ...
                if (!vm.job.params || !vm.job.params.includes) {
                    if (!vm.job.params) {
            ...
            if (vm.job && vm.job.name) {
                if (vm.job.runTime && typeof vm.job.runTime === 'string') {
                    EditorService.toJSON(vm.job.runTime).then(function (res) {
                        ...
                        if (!angular.equals(vm._tempJob, vm.job) && vm._tempJob.name === vm.job.name) {
                            if(!vm.lockedBy) {
        ...
        vm.$on('RELOAD', function (evt, job) {
            if (vm.job && vm.job.name) {
                vm.getFileObject(vm.job, vm.job.path, function (res) {
                    vm.job.current = true;
                    vm._tempJob = angular.copy(vm.job);
                });
            }else{
        ...
        vm.$on('NEW_OBJECT', function (evt, job) {
            vm.lockedBy = job.data.lockedByUser;
            if (job.parent && job.parent.lockedBy && job.parent.lockedBy !== vm.username) {
                vm.lockedBy = job.parent.lockedBy;
            }
            storeObject();
            initialDefaultValue();
            if (job.data.type) {
    ...
    function JobChainEditorCtrl($scope, EditorService, $interval) {
        ...
        vm.removeJobChain = function (jobChain) {
            for (let i = 0; i < vm.jobChains.length; i++) {
                if (vm.jobChains[i].name === jobChain.name) {
                    vm.deleteObject('JOBCHAIN', jobChain.name, jobChain.path);
                    break;
                }
            }
        };
        ...
        vm.$on('NEW_OBJECT', function (evt, jobChain) {
            vm.lockedBy = jobChain.data.lockedByUser;
            if (jobChain.parent && jobChain.parent.lockedBy && jobChain.parent.lockedBy !== vm.username) {
                vm.lockedBy = jobChain.parent.lockedBy;
            }
            storeObject();
    ...
    function OrderEditorCtrl($scope, EditorService, $interval) {
            ...
            if (order) {
                vm.getFileObject(order, order.path, function () {
                    ...
                    if (vm.order.priority)
                        vm.order.priority = parseInt(vm.order.priority);
        ...
        vm.removeOrder = function (order) {
            for (let i = 0; i < vm.orders.length; i++) {
                if (vm.orders[i].name === order.name) {
                    vm.deleteObject('ORDER', order.name, order.path);
                    break;
                }
            }
        };
            ...
            if (title === 'parameter') {
                ...
                EditorService.getFile({
                    ...
                    if (res.configuration && res.configuration.jobChain && res.configuration.jobChain.order) {
                        ...
                        vm._jobChainNodes = res.configuration.jobChain.order.jobChainNodes;
                    }
        ...
        vm.$on('NODE_PARAMETER', function (evt) {
            if (vm.nodeparams && vm.nodeparams.paramList) {
                let _path = '';
                if (vm.order.path === '/') {
                    _path = vm.order.path + vm.order.name;
                } else {
                ...
                EditorService.store({
                    ...
                    configuration: {
                        "jobChain": {
                            "name": vm.order.jobChain,
                            "order": {"params": vm.nodeparams, "jobChainNodes": vm._jobChainNodes}
                        }
                    }
                });
            }
        ...
        vm.addParameter = function () {
            ...
            if (!EditorService.isLastEntryEmpty(vm.order.params.paramList, 'name', 'value')) {
        ...
        vm.getSelectedJobChainData = function (data, path, rename) {
            ...
            if (rename) {
                let name = vm.order.jobChain + ',' + vm.order.orderId;
                let oldPath = '', newPath = '';
                if (path === '/') {
                    newPath = path + name;
                    oldPath = path + vm.order.name;
                } else {
                    newPath = path + '/' + name;
                    oldPath = path + '/' + vm.order.name;
                }
            ...
            if (vm.order && vm.order.name) {
                if (vm.order.runTime && typeof vm.order.runTime === 'string') {
                    EditorService.toJSON(vm.order.runTime).then(function (res) {
                        ...
                        if (!angular.equals(vm._tempOrder, vm.order) && vm._tempOrder.name === vm.order.name) {
                            if(!vm.lockedBy) {
                                vm.lockObject(vm.order);
                            }
                            vm.storeObject(vm.order, vm.order, null, function (result) {
                                if (!result) {
                                    vm.order.deployed = false;
                                    vm._tempOrder = angular.copy(vm.order);
                                }
                            });
        ...
        vm.$on('RELOAD', function (evt, order) {
            if (vm.order && vm.order.name) {
                vm.getFileObject(vm.order, vm.order.path, function (res) {
                    vm.order.current = true;
                    vm._tempOrder = angular.copy(vm.order);
                });
            }else{
                vm.jobChains = order.folders[1].children || [];
                vm.orders = order.folders[2].children || [];
            }
        });
    ...
    function ProcessClassEditorCtrl($scope, $interval) {
        ...
        vm.removeProcessClass = function (processClass) {
            for (let i = 0; i < vm.processClasses.length; i++) {
                if (vm.processClasses[i].name === processClass.name) {
                    vm.deleteObject('PROCESSCLASS', processClass.name, processClass.path);
                    break;
                }
            }
        };
        ...
        function storeObject(isCheck) {
            if (vm.processClass && vm.processClass.name) {
                ...
                if (!angular.equals(vm._tempProcessClass, vm.processClass) && vm._tempProcessClass.name === vm.processClass.name) {
                    if(!vm.lockedBy) {
        ...
        vm.$on('RELOAD', function (evt, processClass) {
            if (vm.processClass && vm.processClass.name) {
                vm.getFileObject(vm.processClass, vm.processClass.path, function (res) {
                    vm.processClass.current = true;
                    vm._tempProcessClass = angular.copy(vm.processClass);
                });
            }else{
        ...
        vm.$on('NEW_OBJECT', function (evt, processClass) {
            vm.lockedBy = processClass.data.lockedByUser;
            if (processClass.parent && processClass.parent.lockedBy && processClass.parent.lockedBy !== vm.username) {
                vm.lockedBy = processClass.parent.lockedBy;
            }
            storeObject();
    ...
    AgentClusterEditorCtrl.$inject = ['$scope', 'EditorService', '$interval'];

    function AgentClusterEditorCtrl($scope, EditorService, $interval) {
        const vm = $scope;
        vm.filter = {'sortBy': 'name', sortReverse: false};
        vm.agentClusters = [];
        ...
        vm.removeAgentCluster = function (agentCluster) {
            for (let i = 0; i < vm.agentClusters.length; i++) {
                if (vm.agentClusters[i].name === agentCluster.name) {
                    vm.deleteObject('AGENTCLUSTER', agentCluster.name);
                    break;
                }
            }
        };
        ...
        function storeObject(isCheck) {
            if (vm.agentCluster && vm.agentCluster.name) {
                ...
                if (!angular.equals(vm._tempAgentCluster, vm.agentCluster) && vm._tempAgentCluster.name === vm.agentCluster.name) {
                    if(!vm.lockedBy) {
        ...
        vm.$on('RELOAD', function (evt, agentCluster) {
            if (vm.agentCluster && vm.agentCluster.name) {
                vm.getFileObject(vm.agentCluster, vm.agentCluster.path, function (res) {
                    vm.agentCluster.current = true;
                    vm._tempAgentCluster = angular.copy(vm.agentCluster);
                });
            }else{
        ...
        vm.$on('NEW_OBJECT', function (evt, agentCluster) {
            vm.lockedBy = agentCluster.data.lockedByUser;
            if (agentCluster.parent && agentCluster.parent.lockedBy && agentCluster.parent.lockedBy !== vm.username) {
                vm.lockedBy = agentCluster.parent.lockedBy;
            }
            storeObject();
    ...
    ScheduleEditorCtrl.$inject = ['$scope', 'EditorService', '$interval'];

    function ScheduleEditorCtrl($scope, EditorService, $interval) {
        const vm = $scope;
        vm.filter = {'sortBy': 'name', sortReverse: false};
        vm.schedules = [];
        ...
        vm.removeSchedule = function (schedule) {
            for (let i = 0; i < vm.schedules.length; i++) {
                if (vm.schedules[i].name === schedule.name) {
                    vm.deleteObject('SCHEDULE', schedule.name, schedule.path);
                    break;
                }
            }
        };
        ...
        function storeObject(isCheck) {
            ...
            if (vm.schedule && vm.schedule.name) {
                ...
                if (!angular.equals(vm._tempSchedule, vm.schedule) && vm._tempSchedule.name === vm.schedule.name) {
                    if(!vm.lockedBy) {
        ...
        vm.$on('RELOAD', function (evt, schedule) {
            if (vm.schedule && vm.schedule.name) {
                vm.getFileObject(vm.schedule, vm.schedule.path, function (res) {
                    vm.schedule.current = true;
                    vm._tempSchedule = angular.copy(vm.schedule);
                });
            }else{
        ...
        vm.$on('NEW_OBJECT', function (evt, schedule) {
            vm.lockedBy = schedule.data.lockedByUser;
            if (schedule.parent && schedule.parent.lockedBy && schedule.data.type) {
                vm.lockedBy = schedule.parent.lockedBy;
            }
            storeObject();
    ...
    function LockEditorCtrl($scope, $interval) {
        ...
        vm.removeLock = function (lock) {
            for (let i = 0; i < vm.locks.length; i++) {
                if (vm.locks[i].name === lock.name) {
                    vm.deleteObject('LOCK', lock.name, lock.path);
                    break;
                }
            }
        };
        ...
        function storeObject(isCheck) {
            if (vm.lock && vm.lock.name) {
                ...
                if (!angular.equals(vm._tempLock, vm.lock) && vm._tempLock.name === vm.lock.name) {
                    if(!vm.lockedBy) {
        ...
        vm.$on('RELOAD', function (evt, lock) {
            if (vm.lock && vm.lock.name) {
                vm.getFileObject(vm.lock, vm.lock.path, function (res) {
                    vm.lock.current = true;
                    vm._tempLock = angular.copy(vm.lock);
                });
            }else{
        ...
        vm.$on('NEW_OBJECT', function (evt, lock) {
            vm.lockedBy = lock.data.lockedByUser;
            if (lock.parent && lock.parent.lockedBy && lock.parent.lockedBy !== vm.username) {
                vm.lockedBy = lock.parent.lockedBy;
            }
            storeObject();
            if (lock.data.type) {
                vm.lock = lock.data;
    ...
    function MonitorEditorCtrl($scope, EditorService, $interval) {
            ...
            if (vm.job && vm.job.monitors) {
                ...
                for (let i = 0; i < vm.monitors.length; i++) {
                    if (vm.monitors[i].name === monitor.name) {
                        vm.deleteObject('MONITOR', monitor.name, monitor.path);
                        break;
                    }
                }
            }
        ...
        function storeObject(isCheck) {
            if (vm.job && vm.job.name) {
                ...
                if (vm.monitor && vm.monitor.name) {
                    ...
                    if (!angular.equals(vm._tempMonitor, vm.monitor) && vm._tempMonitor.name === vm.monitor.name) {
                        if(!vm.lockedBy) {
        ...
        vm.$on('RELOAD', function (evt, monitor) {
            ...
            }else if (vm.monitor && vm.monitor.name) {
                vm.getFileObject(vm.monitor, vm.monitor.path, function (res) {
                    vm.monitor.current = true;
                    vm._tempMonitor = angular.copy(vm.monitor);
                });
            }else{
        ...
        vm.$on('NEW_OBJECT', function (evt, monitor) {
            vm.lockedBy = monitor.data.lockedByUser;
            if (monitor.parent && monitor.parent.lockedBy && monitor.parent.lockedBy !== vm.username) {
                vm.lockedBy = monitor.parent.lockedBy;
            }
            storeObject();
            vm.job = undefined;
    ...
    function CommandEditorCtrl($scope, EditorService) {
        ...
        function detectChanges() {
            watcher1 = $scope.$watchCollection('job.commands', function (newValues, oldValues) {
    ...
    function StepNodeCtrl($scope, $rootScope, $timeout, EditorService, orderBy, $interval) {
                ...
                for (let i = 0; i < vm.jobChain.jobChainNodes.length; i++) {
                    ...
                    if (!nFlag && vm.jobChain.jobChainNodes[i].nextState) {
                        if (vm.jobChain.jobChainNodes.filter(function (x) {
                            return x.state === vm.jobChain.jobChainNodes[i].nextState
                        }).length === 0) {
                            vm.jobChain.jobChainNodes.push({
                    ...
                    if (!eFlag && vm.jobChain.jobChainNodes[i].errorState) {
                        if (vm.jobChain.jobChainNodes.filter(function (x) {
                            return x.state === vm.jobChain.jobChainNodes[i].errorState
                        }).length === 0) {
                            vm.jobChain.jobChainNodes.push({
        ...
        vm.applyFileOrder = function (form) {
            if (vm._tempFileOrder) {
                ...
                console.log('else', vm.fileOrderSource)
        ...
        vm.cancelFileOrder = function (form) {
            ...
            form.$setUntouched();
            form.$invalid = false;
            form.fileOrderSourceDirectory.$invalid = false;
        };
        ...
        function getNodeParams() {
            ...
            EditorService.getFile({
                ...
                vm.jobChainNodes = [];
                if (res.configuration && res.configuration.jobChain && res.configuration.jobChain.order) {
                    if (res.configuration.jobChain.order.jobChainNodes) {
                        vm.jobChainNodes = res.configuration.jobChain.order.jobChainNodes || {};
                    }
                    if (res.configuration.jobChain.order.params) {
                        vm._nodeParams = res.configuration.jobChain.order.params;
                    }
                }
                vm.jobChainNode = {};
        ...
        vm.$on('NODE_PARAMETER', function (evt) {
            if (vm.jobChainNodes && vm.jobChainNodes.length === 0) {
                vm.jobChainNodes.push(vm.jobChainNode);
            }
            let _path = '';
            if (vm.order.path === '/') {
                _path = vm.order.path + vm.order.name;
    ...
    function XMLEditorCtrl($scope, SOSAuth, CoreService, AuditLogService, $location, $http, $uibModal, gettextCatalog, toasty, FileUploader, EditorService, $interval) {
        ...
        function storeXML() {
            vm._xml = _showXml();
            let eRes = compare(vm.prevXML.toString(), vm._xml.toString());
            if (!eRes) {
                EditorService.storeXML({
                    ...
                    vm.XSDState = state;
                    let dateTime = res.modified.split('T');
                    let d = dateTime[0];
                    let t = dateTime[1].split('Z');
                    let xt = t[0];
                    dateTime = d + ' ' +xt;
                    vm.XSDState.modified = dateTime;
                    hideButtons();
                }, function () {
        ...
        function removeComment(data) {
            console.log(data.replace(/\<\!\-\-((?!\-\-\>)[\s\S])*\-\-\>\s*/g, ''));
        ...
        function getRootNode(doc, check) {
            ...
            if (text) {
                temp.text = text;
                if(!temp.attributes || !temp.values) {
                    temp.show = true;
                } else {
                    temp.show = false;
                }
            ...
            if (child.length > 0) {
                for (let i = 0; i < child.length; i++) {
                    if (text && text.parent === child[i].ref) {
                        if (!child[i].text) {
                            child[i].text = text;
                            if(!child[i].attributes && !child[i].values) {
                                child[i].show = true;
                            } else {
                                child[i].show = false;                                
                            }
        ...
        vm.pasteNode = function (node) {
            ...
            node.nodes = _.orderBy(nodeArr.nodes, ['order'], ['asc']);
            vm.cutData = false;
            vm.checkRule = true;
            printArraya(false);
        };
