...
(function () {
    ...
    function JOEEditorCtrl($scope, SOSAuth, CoreService, EditorService, orderBy, $uibModal, clipboard) {
        ...
        vm.$on('SETLASTSELECTION', function (evt, obj) {
            lastClickedItem = obj;
        });
    ...
    function JobEditorCtrl($scope, $uibModal, EditorService, $interval) {
        ...
        vm.createJob = function (job) {
            vm.getFileObject(job, job.path, function () {
                ...
                vm.$emit('SETLASTSELECTION', vm.job);
                detectChanges();
            });
        };
        ...
        vm.createJobChain = function (jobChain) {
            if (jobChain) {
                vm.getFileObject(jobChain, jobChain.path, function () {
                    ...
                    vm.$emit('SETLASTSELECTION', vm.jobChain);
                    detectChanges()
                });
            } else {
        ...
        vm.createOrder = function (order) {
            if (order) {
                vm.getFileObject(order, order.path, function () {
                    ...
                    vm.$emit('SETLASTSELECTION', vm._order);
                    detectChanges();
                });
            } else {
        ...
        vm.createProcessClass = function (processClass) {
            if (processClass) {
                vm.getFileObject(processClass, processClass.path, function () {
                    ...
                    vm.$emit('SETLASTSELECTION', vm.processClass);
                });
            } else {
        ...
        vm.createAgentCluster = function (agentCluster) {
            if (agentCluster) {
                vm.getFileObject(agentCluster, agentCluster.path, function () {
                    ...
                    vm.$emit('SETLASTSELECTION', vm.agentCluster);
                });
            } else {
        ...
        vm.createSchedule = function (schedule) {
            if (schedule) {
                vm.getFileObject(schedule, schedule.path, function () {
                    ...
                    vm.$emit('SETLASTSELECTION', vm.schedule);
                });
            } else {
        ...
        vm.createLock = function (lock) {
            if (lock) {
                vm.getFileObject(lock, lock.path, function () {
                    ...
                    vm.$emit('SETLASTSELECTION', vm.lock);
                });
            } else {
            ...
            if (vm.job && vm.job.monitors) {
                ...
                if (monitor) {
                    vm.getFileObject(monitor, monitor.path, function () {
                        ...
                        vm.$emit('SETLASTSELECTION', vm.monitor);
                    });
                } else {
    ...
    function CommandEditorCtrl($scope, EditorService) {
        ...
        vm.closeSidePanel1 = function () {
            vm.closeSidePanel();
            if (!angular.equals(vm._tempCode, vm.code)) {
                if (vm.code.params && vm.code.params.paramList && vm.code.params.paramList.length === 0) {
    ...
    function StepNodeCtrl($scope, $rootScope, $timeout, EditorService, orderBy, $interval) {
        ...
        function addOverlays(graph, cell) {
            let img = 'images/grey-bar.svg';
            let overlay = new mxCellOverlay(
                new mxImage(img, 10, 10), ""
            );
        ...
        vm.sortJobChainOrderForGraph = function () {
            if(vm.jobChain && vm.jobChain.jobChainNodes && vm.jobChain.jobChainNodes.length>0) {
                let tempArr = [];
                if(vm.jobChain.jobChainNodes[0].job) {
                    tempArr.push(vm.jobChain.jobChainNodes[0]);
                }
                if(tempArr.length>0) {
                    for (let i = 0; i < tempArr.length; i++) {
                        for (let j = 1; j < vm.jobChain.jobChainNodes.length; j++) {
                            if (tempArr[i].nextState === vm.jobChain.jobChainNodes[j].state && vm.jobChain.jobChainNodes[j].job) {
                                tempArr.push(vm.jobChain.jobChainNodes[j]);
                                break;
                            }
                        }
                    }
                    vm.jobChain.jobChainNodes.forEach(function (node){
                        if(!node.job) {
                            tempArr.push(node);
                        }
                    });        
                }
                vm.jobChain.jobChainNodes = tempArr;
            }
        };
        ...
        vm.closeSidePanel1 = function () {
            ...
            if (!angular.equals(vm._tempRc, vm.node)) {
                storeObject();
