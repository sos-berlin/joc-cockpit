...
(function () {
    ...
    DialogCtrl.$inject = ['$scope', '$uibModalInstance', '$window','$uibModal'];
    function DialogCtrl($scope, $uibModalInstance, $window,$uibModal) {
        var vm = $scope;
        vm.error = false;
        if (vm.userPreferences.auditLog) {
            vm.display = true;
    ...
    function FrequencyCtrl($scope, $rootScope, gettextCatalog,$filter, CalendarService) {
        ...
        vm.addCalendarDates = function(){
             if(unselectDates.length>0){
                 checkIncludeExclude(unselectDates);
             }
            if (vm.flag) {
                vm.save();
            }else{
    ...
    function RuntimeEditorDialogCtrl($scope, $rootScope, $uibModalInstance, toasty, $timeout, gettextCatalog, $window, CalendarService) {
      ...
      var selectedMonthsU = [];
        ...
        var calendarIds=[];
        function setCalendarIds(id){
            if(calendarIds.indexOf(id)==-1)
                calendarIds.push(id)
        }
                        ...
                        if (date._calendar) {
                            ...
                            if (vm.selectedCalendar && vm.selectedCalendar.length > 0) {
                                for (var i = 0; i < vm.selectedCalendar.length; i++) {
                                    if (vm.selectedCalendar[i].id == date._calendar) {
                                        flg = true;
                                        break;
                                    }
                                }
                            ...
                            if (!flg) {
                                setCalendarIds(date._calendar);
                            }
                        }
                        if (index == run_time.date.length - 1) {
                            getCalendarById();
                        }
                    });
                }
