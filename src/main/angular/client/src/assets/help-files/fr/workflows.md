...
Le panneau de gauche est organisé en onglets qui permettent de naviguer à partir de dossiers et de filtrer par Tags pour les Workflows et les Ordres.
...
- Le filtrage par étiquette est proposé à partir des onglets suivants :
...
Le résumé de l'Ordre est indiqué pour les Ordres liés aux Workflows affichés pour les dossiers ou les étiquettes sélectionnés.
...
- **No. d'Ordres** indique le nombre d'Ordres assignés au Workflow. 
  - Jusqu'à trois Ordres sont affichés directement avec le Workflow. Ils offrent un menu d'action pour les opérations de l'Ordre.
    - Les utilisateurs peuvent cliquer sur l'ID de l'Ordre indiqué pour afficher le journal de sortie de l'Ordre à partir de [Order Log View](/order-log). Le journal comprend la sortie créée par tous les Jobs exécutés avec le Workflow.
...
Lorsqu'un Workflow est développé à l'aide de l'icône flèche vers le bas disponible pour un Workflow, ses Jobs et Instructions de Workflow seront affichés.

## Panneau Historique

Ce panneau s'affiche dans la partie inférieure de la fenêtre lorsque les utilisateurs cliquent sur le nom du Workflow ou ajoutent un Ordre.
...
- **Identification de l'Ordre** : en cliquant sur l'ID de l'Ordre, vous afficherez le journal de l'Ordre à partir du site [Order Log View](/order-log). Le journal comprend les données créées par tous les Jobs exécutés avec le Workflow.
...
- **Job** indique le nom de la tâche.
- **Label** indique la position de la tâche dans le Workflow.
- **Status** est le résultat de l'exécution du job indiqué par *in progress*, *successful* ou *failed*.
- **Start Time**, **End Time** indiquent le début et la fin de l'exécution du job.
- la **criticité** est spécifiée à l'adresse [Configuration - Inventory - Workflows - Job Options](/configuration-inventory-workflow-job-options) et indique la pertinence d'un travail :
  - *Minoritaire*
  - *Minoritaire* *Normal*
  - *Majeur*
  - *Minoritaire* *Normal* *Majeur* *Critique
- **Code de retour** est le code de sortie d'un job Shell ou le code de retour d'un job JVM. Le panneau [Configuration - Inventory - Workflows - Job Properties](/configuration-inventory-workflow-job-properties) permet de configurer les codes de retour en cas de succès ou d'échec de l'exécution d'un job.

Pour accéder à la sortie du journal, l'action suivante est disponible :

- **Job** : en cliquant sur le *nom du job*, vous afficherez le journal de sortie du job à partir du site [Task Log View](/task-log). 
...
Ce panneau affiche les mêmes informations que le site [Audit Log](/audit-log) en se concentrant sur le Workflow en cours.
...
- **Suspendre tout** agit comme un *arrêt d'urgence* et suspend tous les Workflows, quelle que soit la sélection de Workflows actuellement affichée. Les Workflows suspendus sont gelés, ils acceptent les Ordres mais ne démarrent pas d'Ordres à moins que le Workflow ne soit repris. Les Ordres en cours d'exécution poursuivent le travail en cours ou une autre instruction avant d'être suspendus.
- **Reprendre tout** reprend tous les Workflows suspendus, quelle que soit la sélection de Workflows actuellement affichée.

### Opérations sur les travaux et les instructions de Workflow

Les opérations suivantes sont disponibles pour les travaux à partir du menu d'action correspondant :

- **Skip Job** empêche un Ordre d'exécuter le Job associé et le fait passer à l'instruction de Workflow suivante.
- **Unkskip Job** rétablit un job précédemment ignoré.
- **Arrêter l'Ordre** suspend les Ordres arrivant à l'Ordre. Les ordres peuvent être poursuivis à partir d'une opération *Reprise* qui permet de continuer le traitement à partir d'un autre noeud de Workflow ou de forcer le traitement du Job arrêté.
- **Unstop Job** rétablit un job précédemment arrêté.

### Opérations sur les Ordres

Les utilisateurs disposent d'un menu d'action par Ordre qui propose les opérations suivantes :

- **Annuler** met fin à l'Ordre. *Les Ordres en cours d'exécution* termineront le travail ou l'instruction de Workflow en cours et quitteront le Workflow avec un *état d'historique* *échec*.
- **Les Ordres en cours d'exécution termineront le travail ou l'instruction de flux de travail en cours et quitteront le flux de travail avec un *état de l'historique* *Annuler/terminer la tâche** mettra fin de force aux Ordres en cours d'exécution. Les Ordres quitteront le Workflow avec un *état d'historique* *échec*.
- **Suspendre** suspend l'Ordre. Les Ordres en cours d'exécution seront suspendus une fois qu'ils auront terminé le travail ou l'instruction de Workflow en cours.
...
- **Agents** permet de filtrer les Workflows contenant des Jobs affectés à un ou plusieurs Agents sélectionnés.
...
Le site [Workflows - Search](/workflows-search) offre des critères pour rechercher des Workflows à partir de dépendances, par exemple rechercher des Workflows incluant un nom de travail spécifique, ou utilisant des tableaux d'affichage spécifiques.
