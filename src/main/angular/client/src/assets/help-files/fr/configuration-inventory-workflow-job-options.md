...
L'interface graphique offre un certain nombre d'onglets pour spécifier les détails du tâche. Le deuxième onglet est proposé pour les *options de tâche*.
...
- **Avertir si plus court de** déclenche un avertissement et une notification correspondante si le tâche se termine avant la période spécifiée. Les formats d'entrée sont les suivants :
  ...
  - *30%* : spécifie une période d'exécution 30% plus courte que la moyenne des exécutions précédentes du tâche. Le calcul utilise le site [Task History](/history-tasks) qui est soumis à une purge par le site [Cleanup Service](/service-cleanup).
- **Avertir si plus court de** émet un avertissement et une notification correspondante si le tâche dépasse la période spécifiée. Les formats d'entrée sont les suivants : *1* ou *1s* :
  - *1* ou *1s* : un nombre ou un nombre suivi de *s* spécifie la période d'exécution en secondes.
  - *01:02:03* : spécifie 1 heure, 2 minutes et 3 secondes pour la période d'exécution.
  - *30%* : spécifie une période d'exécution 30% plus longue que la moyenne des exécutions précédentes du tâche. Le calcul utilise le site [Task History](/history-tasks) qui est soumis à une purge par le site [Cleanup Service](/service-cleanup).
...
*Les types d'admission* permettent de spécifier les jours auxquels le tâche peut commencer. En outre, il est possible de spécifier des plages de mois limitant le *type d'admission* à certains mois.
