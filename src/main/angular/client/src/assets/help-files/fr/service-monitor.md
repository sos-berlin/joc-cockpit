...
Le service Monitor est utilisé pour signaler l'état de santé des produits JS7 et pour signaler les problèmes d'exécution du Workflow. Le service Monitor alimente les sous-vues *Monitor* dans JOC Cockpit :

- vérifier la disponibilité des produits JS7 et signaler les sous-vues [Monitor - Controller Availability](/monitor-availability-controller) et [Monitor - Agent Availability](/monitor-availability-agent).
...
Le service Monitor est lancé automatiquement au démarrage de JOC Cockpit. Il peut être redémarré dans la vue Dashboard à partir du rectangle des instances actives de JOC Cockpit en utilisant l'opération *Restart Service - Monitor Service*.
