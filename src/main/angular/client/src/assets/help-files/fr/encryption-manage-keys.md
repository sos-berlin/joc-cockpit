...
La page *Gestion des clés de chiffrement* est utilisée pour gérer les certificats et spécifier les propriétés du certificat.

## Liste des certificats

Les certificats de cryptage existants sont affichés dans une liste :

- le **Menu d'action** permet de mettre à jour et de supprimer l'entrée du certificat.
- **L'alias du certificat** est le nom unique que les utilisateurs attribuent à un certificat.
- **L'icône d'affichage** permet de cliquer sur l'icône pour afficher le certificat correspondant.
- **Chemin d'accès au fichier de la clé privée** spécifie l'emplacement de la clé privée avec les agents associés.

## Opérations sur les certificats

Les boutons suivants sont disponibles en haut de l'écran :

- **Ajouter un certificat** permet d'ajouter un certificat à partir d'un copier/coller. Pour plus d'informations, consultez le site [Manage Encryption Certificate](/encryption-manage-certificate).
- **Importer un certificat** permet de télécharger un fichier de certificat.
...
- **Mettre à jour le certificat** permet de modifier l'entrée du certificat. Pour plus de détails, consultez le site [Manage Encryption Certificate](/encryption-manage-certificate).
