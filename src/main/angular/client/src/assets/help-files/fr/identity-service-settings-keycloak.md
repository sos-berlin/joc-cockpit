...
- **L'URL Keycloak** est l'URL de base pour laquelle l'API REST Keycloak est disponible
- **Compte administratif Keycloak** est un compte Keycloak avec un rôle administratif auquel sont attribués les rôles *realm-management.view-clients* et *realm-management.view-users* dans Keycloak. Le compte administratif est utilisé pour récupérer les rôles d'un compte Keycloak et pour renouveler les jetons d'accès.
- **Keycloak Administrative Password** est le mot de passe du *compte d'administration Keycloak*.
- **Le chemin d'accès au Truststore de Keycloak** indique l'emplacement d'un Truststore si le serveur Keycloak est configuré pour des connexions HTTPS. Le Truststore indiqué doit inclure un certificat X.509 spécifié pour l'utilisation de la clé étendue de l'authentification du serveur.
  ...
  - Si le serveur Keycloak est utilisé pour des connexions HTTPS et que ce paramètre n'est pas spécifié, le Cockpit JOC utilisera le truststore configuré dans le fichier de configuration *JETTY_BASE/resources/joc/joc.properties*. Cela inclut l'utilisation des paramètres relatifs au mot de passe et au type de magasin fiduciaire.
...
- **Keycloak Truststore Password** spécifie le mot de passe qui protège le Truststore si le serveur Keycloak est configuré pour les connexions HTTPS.
- **Keycloak Truststore Type** est l'un de *PKCS12* ou *JKS* (obsolète). Ce paramètre est utilisé si le serveur Keycloak est configuré pour les connexions HTTPS.
- Les clients Keycloak sont des entités qui demandent à Keycloak d'authentifier un compte utilisateur. Par exemple, une application telle que JOC Cockpit agit en tant que client du serveur Keycloak. Les clients utilisent Keycloak pour s'authentifier et fournir une solution d'authentification unique.
  - **L'ID du client Keycloak** et le secret du client Keycloak* sont utilisés pour
  ...
  - **Le secret du client Keycloak** est détenu par le client et doit être connu à la fois par le serveur Keycloak et le Cockpit JOC.
