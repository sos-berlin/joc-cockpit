...
Les notifications sont configurées à partir de la page [Configuration - Notification](/configuration-notification) et peuvent être émises en cas de succès, d'avertissement ou d'erreur dans l'exécution d'un Ordre ou d'un Travail.
...
- **Ordre ID** spécifie l'identifiant unique d'un Ordre.
- **Job** est indiqué si l'avertissement ou l'erreur a été causé par un Job.
- **Type** est l'un des éléments suivants
  - **SUCCESS** qui indique que l'Ordre a été exécuté avec succès, à condition que les notifications soient configurées pour signaler cet état.
  - **WARNING** qui est déclenché par des jobs Shell pour lesquels des codes de retour spécifiques sont configurés pour être des *Avertissements* qui n'affecteront pas le flux d'un Ordre mais qui déclencheront la Notification correspondante.
  - **ERROR** qui peut être levée par des Jobs ou d'autres instructions de Workflow. La notification est déclenchée indépendamment du fait qu'un Workflow puisse appliquer une gestion d'erreur comme dans l'instruction *Try/Catch* ou *Retry* qui permettra à un Ordre de continuer dans le Workflow.
  - **RECOVERED** qui indique qu'un Ordre précédemment *défaillant* s'est rétabli et a été traité avec succès dans le Workflow.
...
- **Successful** limite l'affichage aux notifications relatives à l'exécution réussie d'un Ordre.
- **Failed** limite l'affichage des notifications aux Ordres qui ont *échoué*.
- **Avertissement** limite l'affichage des notifications aux Ordres qui ont provoqué des avertissements.
- **Récupéré** limite l'affichage des notifications aux Ordres qui ont d'abord échoué, puis se sont rétablis en poursuivant avec succès le Workflow.
- **Accusé de réception** limite l'affichage aux notifications qui ont déjà fait l'objet d'un accusé de réception dans le cadre de l'opération concernée.
