...
Le panneau *Plan Quotidien* fournit des informations sur l'exécution des Ordres créés par le Plan Quotidien. Cela exclut les Ordres créés sur demande par l'intervention de l'utilisateur et les Ordres créés à partir d'un fichier surveillé par *File Ordre Sources*.
...
En cliquant sur le nombre d'Ordres indiqué, vous accédez à la vue *Plan journalier* qui affiche les Ordres en détail.

## Filtres

Le bouton déroulant situé dans le coin supérieur droit du panneau permet de sélectionner des Ordres dans une plage de dates :

- **Today** Les ordres sont liés à la journée en cours qui est calculée à partir du fuseau horaire dans le profil de l'utilisateur.
