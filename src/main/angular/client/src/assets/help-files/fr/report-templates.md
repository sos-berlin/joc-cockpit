...
- L'exécution d'un job Shell est considérée comme ayant échoué en fonction du code de sortie du job et éventuellement de la sortie sur le canal stderr.
- L'exécution d'un job JVM est considérée comme un échec sur la base du résultat du job qui peut contenir des exceptions.

## Modèle de rapport : Top n Agents avec le plus grand/le plus petit nombre d'exécutions parallèles de Jobs
...
Le modèle de rapport comptabilise les échecs d'exécution des travaux dont la criticité est élevée. La criticité est un attribut du travail, voir JS7 - Job Instruction.

Le comptage s'effectue de la même manière que pour le modèle de rapport : Top n Jobs avec le plus grand/le plus petit nombre d'échecs d'exécution.
...
## Modèle de rapport : Top n Jobs avec le plus grand/le plus petit besoin de temps d'exécution
...
Pour connaître les raisons possibles de l'échec d'un travail, reportez-vous au modèle de rapport : Top n Jobs avec le plus grand/le plus petit nombre d'échecs d'exécution.
