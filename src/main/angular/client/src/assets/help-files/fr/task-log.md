...
- **stdout** spécifie si la sortie écrite par le job sur le canal stdout sera affichée à l'aide du qualificateur [STDOUT].
- **Debug** s'applique aux travaux Java qui utilisent [JS7 - Job API](https://kb.sos-berlin.com/display/JS7/JS7+-+Job+API). Ces travaux peuvent se voir ajouter l'argument *log_level* contenant la valeur *debug* ou *trace*. Si une sortie de débogage est disponible pour un travail, elle sera affichée à l'aide du qualificateur [DEBUG].
