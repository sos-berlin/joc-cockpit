...
Le déploiement d'objets consiste à les transférer vers un ou plusieurs contrôleurs. Cela s'applique aux objets tels que les Workflows et les ressources de travail disponibles dans le dossier système *Contrôleur*.
...
- **Mettre à jour le Plan Quotidien**
  ...
  - **La sélection de l'option Date de début ajoute un champ de saisie pour la date cible à partir de laquelle le Plan Quotidien sera mis à jour.
...
L'option **Handle recursively** permet d'ajouter des objets de sous-dossiers de manière récursive au déploiement.
...
- Si une ressource d'emploi est référencée par un Workflow déployé et qu'elle doit être révoquée ou supprimée, le Workflow doit également être révoqué ou supprimé.
