...
Le panneau de gauche permet de filtrer les étiquettes des Workflows et des Ordres qui ont déclenché l'exécution du travail.
...
Les balises sont sélectionnées à partir des icônes + et - et peuvent être recherchées à l'aide de l'icône de recherche. L'affichage des étiquettes doit être activé à partir de la page [Settings - JOC Cockpit](/settings-joc).
...
- **Nom du job** indique le job en question.
- **Workflow** indique le Workflow pour lequel le travail a été exécuté.
...
- **L'étiquette** indique la position du travail dans le Workflow. Les utilisateurs attribuent des *étiquettes* aux travaux qui seront affichés. Si le même travail apparaît plus d'une fois dans un Workflow, il sera indiqué par des *étiquettes* différentes.
- **L'historique** indique le résultat du travail.
  - Si les travaux sont terminés, le *Historique Status* sera soit *successful* (réussi), soit *failed* (échoué).
  - Si les travaux ne sont pas terminés, l'état de l'historique sera *en cours*.

### Accès à la sortie du journal

- **Nom du travail** : En cliquant sur le *Nom du travail*, vous afficherez le journal de sortie du travail à partir du site [Task Log View](/task-log).
...
- **Ajouter le travail à la liste des tâches ignorées** pour masquer définitivement le travail à l'écran. Cela peut s'avérer utile pour les tâches exécutées de manière répétée qui alimentent l'Historique des tâches.
- **Ajouter le Workflow à la liste d'ignorance** pour masquer définitivement l'affichage des travaux du Workflow. Cela peut être utile pour les Workflows cycliques qui alimentent l'Historique des tâches.
...
Les utilisateurs peuvent appliquer les filtres disponibles en haut de la fenêtre pour limiter l'affichage des travaux.

- les boutons de filtrage **Successful**, **Failed**, **In Progress** limitent l'affichage aux travaux ayant le *Historique Status* correspondant.
- les boutons de filtrage **Date Range** permettent de choisir la plage de dates pour l'affichage des travaux.
- **La case à cocher "Contrôleur actuel" limite l'affichage des travaux au contrôleur actuellement sélectionné.
