...
- **Job Resource** contient les paramètres de connexion au serveur de messagerie. Pour plus de détails, voir [JS7 - eMailDefault Job Resource](https://kb.sos-berlin.com/display/JS7/JS7+-+eMailDefault+Job+Resource).
...
- **Mail de demande d'approbation**
  ...
  - **L'objet**, le corps** du courrier peuvent inclure des caractères de remplacement qui seront substitués lors de l'envoi du courrier. Les caractères génériques sont spécifiés en utilisant le format $\{placeholder\}.
    - Les caractères génériques suivants sont disponibles :
      ...
      - $\{Category\} : Catégorie
      - $\{Reason\} : Raison
    - En outre, les caractères génériques suivants peuvent être utilisés s'ils sont spécifiés à partir d'une ressource de travail telle que *eMailDefault*.
