...
- **Format du fichier** spécifie soit .zip soit .tar.gz pour le type de compression.
- **Type d'exportation** est l'une des options suivantes
...
- **Filtre
  ...
  - **Draft** inclut les objets à l'état de brouillon.
  - **Deployed** inclut les objets tels que les Workflows et les Job Resources en statut déployé.
  - **Libéré** inclut les objets tels que les planifications et les calendriers en statut libéré. 
...
- **Traiter de manière récursive** permet d'ajouter des objets de sous-dossiers de manière récursive à l'archive d'exportation.

## Types d'exportation

Le **Type d'exportation** permet de sélectionner des objets individuels, des objets provenant de dossiers et des objets provenant de modifications.

### Exportation d'objets individuels

Le *type d'exportation* permet de sélectionner des objets individuels dans la liste des objets affichés.

<img src="export-object.png" alt="Export Object" width="600" height="580" />

### Exportation d'objets à partir de dossiers

Le *Type d'exportation* permet de limiter l'exportation à certains types d'objets de planification tels que les Workflows ou les Plannifications. Les utilisateurs peuvent sélectionner les types d'objets souhaités qui seront ajoutés au fichier d'archive d'exportation.

<img src="export-folder.png" alt="Export Folder" width="600" height="580" />

### Exportation d'objets à partir de modifications

Le *Type d'exportation* permet de sélectionner une modification dans la liste [Changes](/changes). L'exportation sera limitée aux objets liés à la modification.

<img src="export-change.png" alt="Export Change" width="600" height="320" />

## Inclure les dépendances

Les objets de l'inventaire sont liés par des dépendances, voir [Dependency Matrix](/dependencies-matrix). Par exemple, un Workflow référençant une ressource de travail et un verrou de ressource ; une Plannification référençant un calendrier et un ou plusieurs Workflows.

Lors de l'exportation d'objets, la cohérence est prise en compte, par exemple :

- Si un Workflow fait référence à une ressource de travail, le Workflow et la ressource de travail peuvent être exportés, même s'ils sont stockés dans des dossiers sans rapport avec le dossier sélectionné.
- Si une Plannification fait référence à un Calendrier et doit être exportée, la Plannification et le Calendrier peuvent tous deux être exportés.
