...
- **AdmissionTime Anweisung** hält die Ausführung eines Auftrags in einem Arbeitsablauf an, bis das angegebene Zeitfenster erreicht ist. Der Auftrag erhält den Status *Wartend*. Darüber hinaus können Aufträge abgebrochen werden, wenn sie das angebbare Zeitfenster überschreiten. Die Anweisung kann so konfiguriert werden, dass ein Auftrag alle enthaltenen Anweisungen überspringt, falls kein passendes Zeitfenster für das Startdatum des Auftrags gefunden wird. Einzelheiten finden Sie unter [[JS7 - AdmissionTime Instruction](https://kb.sos-berlin.com/display/JS7/JS7+-+AdmissionTime+Instruction).
