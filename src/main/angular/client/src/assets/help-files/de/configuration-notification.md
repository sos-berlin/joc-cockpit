...
- **Nachricht**
  - Eine *Message* definiert den Inhalt, der z.B. per E-Mail an einen Benutzer gesendet wird oder der zur Parametrisierung eines Kommandozeilenprogramms verwendet wird, wie z.B. der Inhalt, der an einen Systemmonitor weitergeleitet werden soll.
    - *Nachrichten* für die Verwendung mit E-Mail stellen den E-Mail-Textkörper dar, der aus einfachem Text oder aus HTML verwendet wird.
    ...
    - *Message*-Elemente können Monitorvariablen enthalten, die Platzhalter für Werte sind, z.B. für den Workflow-Pfad, die Bestell-ID usw.
...
- **MailFragment**
  - Die folgenden Elemente sind für den Versand von Mails erforderlich:
  ...
  - Die folgenden Elemente sind optional, um eine E-Mail zu versenden:
    ...
    - **Von**: Die E-Mail-Adresse des Kontos, das zum Versenden von E-Mails verwendet wird. Beachten Sie, dass die Konfiguration Ihres Mailservers bestimmt, ob ein bestimmtes oder ein beliebiges Konto verwendet werden kann.
- **CommandFragment**
  - **MessageRef**: Gibt den Verweis auf ein *Message*-Element an, das den Inhalt liefert, der mit dem Command-Element weitergeleitet wird. Der Inhalt der Nachricht ist in der *$\{MESSAGE\}* Monitorvariable für die Verwendung mit späteren Elementen verfügbar.
  - **Command**: Gibt den Shell-Befehl für Linux/Windows an, der zur Weiterleitung von Benachrichtigungen verwendet wird, z.B. an ein System Monitor-Dienstprogramm.
    - Zum Beispiel kann der folgende Shell-Befehl verwendet werden:
      - *echo "$\{MESSAGE\}" &gt;&gt; /tmp/notification.log*
      - Der Shell-Befehl *echo* fügt den Inhalt der *$\{MESSAGE\}* Monitor-Variable an eine Datei im Verzeichnis */tmp* an.
- **JMSFragment**
...
- **SystemBenachrichtigung**: Wählt eines oder mehrere der oben genannten *MonitorFragmente* aus. Es ist möglich, eine Reihe von *Fragmenten* desselben Fragmenttyps auszuwählen.
  - Benachrichtigungen werden aus Systemfehlern und Warnungen erstellt, die in den JS7-Produktprotokolldateien identifiziert werden, siehe [Log Notification Service](/service-log-notification).
  - Das Element wird verwendet, um die Unteransicht [Monitor - System Notifications](/monitor-notifications-system) von JOC Cockpit zu füllen.
