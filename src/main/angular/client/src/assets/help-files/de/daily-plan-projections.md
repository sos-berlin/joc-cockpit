...
Die Projektionen beziehen sich auf die Daten des Tagesplans, nicht auf Kalenderdaten. 
...
- Zeitzonen
  - Wenn Zeitpläne andere Zeitzonen verwenden als der Tagesplan, können sich die Startzeiten der Orders mit dem vorherigen oder nächsten Tag überschneiden. Solche Orders werden mit dem entsprechenden Tagesplan-Datum angezeigt, zeigen aber Startzeiten für ein anderes Datum an.
...
Alle Daten und Zeiten werden in der Zeitzone angezeigt, die im Profil des Benutzers angegeben ist.
...
- **Orange Aufträge**: Sie zeigen geplante Aufträge an, die auf der Grundlage von Startzeitregeln berechnet wurden.
- **Invertierte Projektion**:
  - *unmarkiert*: Die Projektion zeigt die Tage an, für die die Aufträge berechnet werden, sowie die Anzahl der Aufträge. Sie können auf einen einzelnen Tag klicken, um die Startzeiten der Orders zu ermitteln.
  - *Aktiviert*: Wenn Sie die Projektion umkehren, werden die Tage hervorgehoben, für die es Zeitpläne gibt, die keine Aufträge erstellen werden. Wenn Sie auf den entsprechenden Tag klicken, werden Zeitpläne ohne Orders angezeigt.

### Erweiterter Filter

Der Filter bietet die Möglichkeit, die Anzeige von Aufträgen auf bestimmte Ordner mit Workflows oder Zeitplänen zu beschränken.

## Operationen auf Projektionen

### Projektionen erstellen

- Hochrechnungen werden vom Daily Plan Service während seines täglichen Laufs berechnet. Spätere Änderungen am Tagesplan während des Tages werden nicht berücksichtigt.
- Sie können die Projektionen bei Bedarf über die entsprechende Schaltfläche neu erstellen.
- Das *Erhebungsdatum* zeigt das Erstellungsdatum der aktuellen Tagesplan-Projektion an.

### Projektionen exportieren

Projektionen können in eine .xlsx-Datei exportiert werden, wobei das Datum des Tagesplans auf der x-Achse und der Workflow und der Zeitplan auf der y-Achse angezeigt werden.

- Mit der Tastenkombination *Exportieren* exportieren Sie die im Fenster sichtbaren Aufträge. 
- Über die Schaltfläche *Exportieren* können Sie die zu exportierenden Aufträge auswählen:
  ...
  - **Invertierte Projektion**: 
    - *Unmarkiert*: Exportiert Daten, für die Aufträge berechnet werden.
    - *Aktiviert*: Exportiert Termine, für die keine Aufträge berechnet werden. Damit können Sie überprüfen, ob arbeitsfreie Tage berücksichtigt werden.
