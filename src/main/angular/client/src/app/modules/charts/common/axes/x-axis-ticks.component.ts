...
@Component({
  ...
  template: `
    <svg:g #ticksel>
      <svg:g *ngIf="doubleSideAxis">
        <svg:g *ngFor="let tick of ticks" class="tick" [attr.transform]="tickTransformTop(tick)">
      ...
      <svg:g *ngFor="let tick of ticks" class="tick" [attr.transform]="tickTransformBottom(tick)">
        <title>{{ tickFormat(tick) }}</title>
        <svg:text
          stroke-width="0.01"
          [attr.text-anchor]="textAnchor"
...
export class XAxisTicksComponent implements OnChanges, AfterViewInit {
  ...
  tickTransform(tick: number): string {
    return 'translate(' + this.adjustedScale(tick) + ',' + (this.verticalSpacing + (this.doubleSideAxis ? 8 : 0)) + ')';
  }

  tickTransformBottom(tick: number): string {
    return 'translate(' + this.adjustedScale(tick) + ',' + (this.verticalSpacing + (this.doubleSideAxis ? 2 : 0)) + ')';
  }
