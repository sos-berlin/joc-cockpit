  ...
  groupBy(data): any {
    ...
    data.forEach((value) => {
      ...
      if (value.healthState) {
        label = value.healthState._text;
        result.color = this.coreService.getColorBySeverity(value.healthState.severity, false);
        result.hoverColor = this.coreService.getColorBySeverity(value.healthState.severity, true);
        ...
        this.mapObj.set(result.count + ' ' + result._text, value.healthState._text);
        results.push(result);
      }
    });
    return results;
