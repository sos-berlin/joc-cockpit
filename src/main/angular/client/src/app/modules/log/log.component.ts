...
export class LogComponent implements OnInit, OnDestroy {
  ...
  public init() {
    this.loading = true;
    if (this.coreService.getParam('historyId')) {
      this.orderId = this.coreService.getParam('orderId');
      this.loadOrderLog();
    } else if (this.coreService.getParam('taskId')) {
      this.taskId = this.coreService.getParam('taskId');
      this.loadJobLog();
    } else {
      alert('Invalid URL');
    }
  ...
  loadOrderLog() {
    this.jobChain = this.coreService.getParam('jobChain');
    let orders: any = {};
    orders.jobschedulerId = this.coreService.getParam('schedulerId');
    orders.jobChain = this.jobChain;
    orders.orderId = this.orderId;
    orders.historyId = this.coreService.getParam('historyId');
  ...
  loadJobLog() {
    this.job = this.coreService.getParam('job');
    let jobs: any = {};
    jobs.jobschedulerId = this.coreService.getParam('schedulerId');
  ...
  downloadLog() {
    this.cancel();
    const schedulerId = this.coreService.getParam('schedulerId');
    if (this.coreService.getParam('orderId')) {
      const orderId = this.coreService.getParam('orderId');
      const jobChain = this.coreService.getParam('jobChain');
      const historyId = this.coreService.getParam('historyId');
      ...
      const taskId = this.coreService.getParam('taskId');
  ...
  saveUserPreference() {
    ...
    configObj.id = parseInt(window.sessionStorage.preferenceId, 10);
