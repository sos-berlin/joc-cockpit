...
export class PermissionModal {
    ...
    checkCovered(currentPermission) {
        ...
        let self = this;
        this.rolePermissions.forEach(function (permission1) {
        ...
        let self = this;
         if(this.add) {
             ...
             for (let i = 0; i < this.rolePermissions.length; i++) {
                 if (self.oldPermission === self.rolePermissions[i] || _.isEqual(self.oldPermission, self.rolePermissions[i])) {
                     self.rolePermissions[i] = obj;
                     break;
                 }
             }
         }
...
export class FolderModal implements OnInit {
    submitted:boolean = false;
    ...
    onSubmit(obj):void {
        ...
        if (!this.newFolder) {
            ...
            for (let i = 0; i < this.folderArr.length; i++) {
                if (self.oldFolder === self.folderArr[i] || _.isEqual(self.oldFolder, self.folderArr[i])) {
                    self.folderArr[i] = obj;
                    break;
                }
            }
        } else {
            ...
            if (this.folderObj.paths && this.folderObj.paths.length > 0) {
                this.folderObj.paths.forEach(function (path) {
                    self.folderArr.push({folder: path, recursive: obj.recursive});
                });
            }
        }
        for (let i = 0; i < this.userDetail.masters.length; i++) {
            ...
            if (_.isEqual(master.master, self.masterName) || (master.master == '' && self.masterName == 'default')) {
                master.roles.forEach(function (value) {
                    if (_.isEqual(value.role, self.roleName)) {
                        self.rolePermissions = value.permissions;
                        self.folderArr = value.folders;
                        self.originalPermission = _.clone(self.rolePermissions);
                    }
                });
            }
        });
    ...
    preparePermissionOptions() {
        ...
        temp.forEach(function (option, index) {
            if (index > 0 && (option.split(':')[2] != temp[index - 1].split(':')[2] || option.split(':')[3] != temp[index - 1].split(':')[3])) {
                self.permissionOptions.push('---------------------------------------------------------------------------------');
            }
            self.permissionOptions.push(option);
        });
        this.permissionOptions = self.permissionOptions;
    }
