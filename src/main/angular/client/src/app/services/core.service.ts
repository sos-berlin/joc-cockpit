...
export class CoreService {
  ...
  private init(): void {
    ...
    this.tabs._help = {language: localStorage['$SOS$LANG']};
  ...
  getTimeZone(): string {
    ...
    return this.normalizeTimeZone(guessedZone);
  }

  private normalizeTimeZone(zone: string): string {
    ...
    return deprecatedToCanonical[zone] || zone;
  }

private cachedTimezones: string[] | null = null;

getTimeZoneList(callback?: (timezones: string[]) => void): any {
  if (this.cachedTimezones) {
    if (callback) {
      callback(this.cachedTimezones);
    }
    return this.cachedTimezones;
  }

  const url = 'utilities/timezones';
  this.post(url, {}).subscribe({
    next: (response: any) => {
      if (response && response.timezones) {
        this.cachedTimezones = response.timezones;
        if (callback) {
          callback(this.cachedTimezones);
        }
      }
...
// Helper method to convert object to array
  ...
  getPeriodStr(period, skip = false): string {
    ...
    if (period.singleStart) {
      ...
      periodStr = periodStr + ' ' + this.translate.instant('runtime.label.every') + ' ' + this.getTimeInString(period.repeat);
    }
    return periodStr;
  }
  ...
  getTimeInString(time: any): string {
    if (time.toString().substring(0, 2) === '00' && time.toString().substring(3, 5) === '00') {
      return time.toString().substring(6, time.length) + ' ' + this.translate.instant('inventory.label.seconds');
    } else if (time.toString().substring(0, 2) === '00') {
      return time.toString().substring(3, time.length) + ' ' + this.translate.instant('inventory.label.minutes');
      ...
      return time.toString().substring(0, 5) + ' ' + this.translate.instant('inventory.label.hours');
    } else {
      return time;
    }
  }
